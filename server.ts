...
async function startServer() {
  ...
  // POST /api/ats/analyze: Full Hybrid ATS Scoring & Evidence Generator
  app.post('/api/ats/analyze', async (req, res) => {
    try {
      ...
      // Step 1: Run pure deterministic scoring engine (Never hallucinated)
      const baseAnalysis = evaluateResumeAgainstJob(resume, job, customWeights);

      // Step 2: If Gemini API is available, generate AI explanation & contextual feedback
...
`;
  ...
  // POST /api/ats/optimize: Actionable Resume Optimization for Target Job
...
`;
  ...
  // POST /api/ats/generate-questions: Grounded AI Interview Intelligence
  app.post('/api/ats/generate-questions', async (req, res) => {
    try {
      const { resume, job } = req.body as { resume: StructuredResume; job: JobRequirement };
      const ai = getGeminiClient();
